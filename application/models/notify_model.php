...
class Notify_model extends CI_Model {
	...
	public function compte_cree($id_user, $PIN)
	{
	...
	public function pin_recree($id_user, $PIN)
	{
		// On selectionne la derniere reservation effectuée par l'utilisateur
		$query = $this->db->query("SET lc_time_names = 'fr_FR'");
		$query = $this->db->query(" SELECT ui.firstname, ui.`email`, ua.PIN FROM user_info AS ui 
	...
	public function reservation_confirmee($id_resa)
	{
		...
		$mail->AddAddress($To , $ToName);
		$ayant_droits_admin = $this->user->get_ayant_droits_admin();
		foreach($ayant_droits_admin as $adm) {
			$mail->AddCC($adm->email , $adm->nom);
		}
		//$mail->AddCC('mravaatanga@cm.perenco.com', 'Marie-Roselyne AVA ATANGA');	
		//$mail->AddCC('embussiotto@cm.perenco.com', 'Tarfa HAMADOU');	
		$mail->AddBCC('passontia@cm.perenco.com', 'Patient Assontia');
